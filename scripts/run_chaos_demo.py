...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--sim-time", type=float, default=100.0, help="Simulation time for the passive motion.")
    parser.add_argument("--theta1", type=float, default=0.5, help="Initial angle of the first link [rad].")
...
def main() -> None:
    ...
    print(sim.state[-1])
    print("\nPeak cart motion:")
    print(np.max(np.abs(sim.state[:, 0])))
