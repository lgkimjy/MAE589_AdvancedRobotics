...
class AnimationOptions:
    ...
    spring_line_width: float = 2.0
    figure_size: tuple[float, float] = (9.0, 5.5)
    follow_cart: bool = False
    camera_width: float = 4.0
    y_limits: tuple[float, float] = (-1.15, 1.35)
    track_bounds: tuple[float, float] | None = None
    goal_x: float | None = None
    history_window: int = 80
    trace_window: int = 80
    spring_coils: int = 7
    spring_width: float = 0.055
    spring_end_fraction: float = 0.16


def _spring_polyline(anchor: np.ndarray, tip: np.ndarray, coils: int, width: float, end_fraction: float) -> np.ndarray:
    direction = tip - anchor
    length = float(np.linalg.norm(direction))
    if length < 1e-9:
        return np.vstack([anchor, tip])

    tangent = direction / length
    normal = np.array([-tangent[1], tangent[0]], dtype=float)
    lead = min(end_fraction * length, 0.16)
    usable_length = max(length - 2.0 * lead, 0.0)

    points = [anchor, anchor + tangent * lead]
    if usable_length < 1e-9:
        points.append(tip)
        return np.asarray(points, dtype=float)

    zigzag_start = anchor + tangent * lead
    zigzag_end = tip - tangent * lead
    zigzag_direction = zigzag_end - zigzag_start
    segment_count = max(coils * 2, 2)
    for idx in range(1, segment_count):
        phase = idx / segment_count
        lateral = width * (1.0 if idx % 2 else -1.0)
        points.append(zigzag_start + phase * zigzag_direction + lateral * normal)
    points.extend([zigzag_end, tip])
    return np.asarray(points, dtype=float)
...
def animate_simulation(
    ...
    x_extent = max(opts.camera_width / 2.0, params.max_length + 0.8)
    ...
    if opts.track_bounds is not None:
        ...
        world_x_min = min([float(cart_positions.min())] + goal_candidates) - params.max_length - 0.4
        world_x_max = max([float(cart_positions.max())] + goal_candidates) + params.max_length + 0.4
    ...
    ax.set_xlabel("x [m]")
    ax.set_ylabel("y [m]")
    ax.set_title("Spring Pendulum Cart-Pole")
    ...
    spring_line, = ax.plot([], [], color="#50514f", linewidth=opts.spring_line_width)
    ...
    if opts.goal_x is not None:
        ax.axvline(opts.goal_x, ymin=0.45, ymax=0.8, color="#1982c4", linewidth=2.0, linestyle="--", alpha=0.9)
        ax.text(opts.goal_x, 0.85, "goal", color="#1982c4", ha="center", va="bottom")
    ...
    def update(frame_idx: int) -> tuple[object, ...]:
        ...
        spring_shape = _spring_polyline(
            cart,
            tip,
            coils=opts.spring_coils,
            width=opts.spring_width,
            end_fraction=opts.spring_end_fraction,
        )
        spring_line.set_data(spring_shape[:, 0], spring_shape[:, 1])
        ...
        if opts.show_history:
            ...
            tip_history.set_offsets(hist[:, 1, :])
            cart_history.set_facecolors(
                np.column_stack([np.full(hist.shape[0], 1.0), np.zeros(hist.shape[0]), np.zeros(hist.shape[0]), alpha])
            )
            tip_history.set_facecolors(
                np.column_stack(
                    [
                        np.full(hist.shape[0], 1.0),
                        np.full(hist.shape[0], 202.0 / 255.0),
                        np.full(hist.shape[0], 58.0 / 255.0),
                        alpha,
                    ]
                )
            )
