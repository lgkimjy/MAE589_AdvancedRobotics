...
def rollout_open_loop(
    ...
    x0[2] = float(np.clip(x0[2], params.min_length, params.max_length))
    ...
    state_hist[0] = x0
    predicted_hist: list[np.ndarray | None] = []
    predicted_sample_hist: list[np.ndarray | None] = []
    ...
    for idx in range(time_grid.size - 1):
        ...
        sol = solve_ivp(
            lambda t, state: spring_cartpole_dynamics(t, state, control_now, params),
        ...
        next_state = sol.y[:, -1]
        next_state[2] = float(np.clip(next_state[2], params.min_length, params.max_length))
        if next_state[2] in (params.min_length, params.max_length):
            next_state[5] = 0.0
